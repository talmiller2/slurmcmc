...
def test_slurmpool_localmap_history(work_dir, verbosity):
    slurm_pool = SlurmPool(work_dir, cluster='local-map', verbosity=verbosity)
    fun = lambda x: x ** 2
    points_1 = [2, 3, 4]
...
def test_slurmpool_localmap_history_with_failed_points(work_dir, verbosity):
    slurm_pool = SlurmPool(work_dir, cluster='local-map', verbosity=verbosity)
    fun = lambda x: x ** 2
...
def test_slurmpool_localmap_2params(work_dir, verbosity):
    slurm_pool = SlurmPool(work_dir, cluster='local-map', verbosity=verbosity)
...
def test_slurmpool_localmap_history_2params(work_dir, verbosity):
    slurm_pool = SlurmPool(work_dir, cluster='local-map', verbosity=verbosity)
    fun = lambda x: x[0] ** 2 + x[1] ** 2
...
def test_slurmpool_localmap_with_budget(work_dir, verbosity):
    slurm_pool = SlurmPool(work_dir, cluster='local-map', verbosity=verbosity, budget=2)
    fun = lambda x: x ** 2
    points = [2, 3, 4, 5, 6]
    res_expected = [fun(point) for point in points]
    res = slurm_pool.map(fun, points)
...
def test_slurmpool_localmap_2params_3outputs(work_dir, verbosity):
    slurm_pool = SlurmPool(work_dir, cluster='local-map', verbosity=verbosity)
...
def test_slurmpool_localmap_2params_3outputs_history_with_failed_points(work_dir, verbosity):
    slurm_pool = SlurmPool(work_dir, cluster='local-map', verbosity=verbosity)
